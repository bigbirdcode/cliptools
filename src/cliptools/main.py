...
def _try_delegate_to_existing_instance(args: list[str]) -> socket.socket | bool:
    """
...
def _delegate_to_existing_instance(args: list[str]) -> bool:
    """
...
def main() -> None:
    ...
    # Check first run by the existence of user files and create them when needed
    ...
    control = controller.Controller(server_socket, user_folder, sys.argv[1:])
    control.start()
